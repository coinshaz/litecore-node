...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'unitedcore-lib': '^' + bitcore.version,
    'unitedcore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/unitedcore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the unitedcore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the unitedcore-node directory and configuration
