...
var bitcore = require('unitedcore-lib');
var zmq = require('zeromq');
var async = require('async');
var LRU = require('lru-cache');
var BitcoinRPC = require('unitedcoind-rpc');
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of unitedcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of unitedcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of unitedcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to bitcoind from unitedcore. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bitcoind is undergoing a reindex. ' +
             ...
             'of unitedcore-node services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/unitedcoind.pid';
