...
`unitedcore-node@2.1.1` to `unitedcore-node@3.0.0`
...
**Important**: Once changes have been made you'll also need to add the `reindex=1` option **only for the first startup** to regenerate the indexes. Once this is complete you should be able to remove the `unitedcore-node.db` directory with the old indexes.
